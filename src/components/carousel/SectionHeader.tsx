...
const SectionHeader: React.FC<SectionHeaderProps> = ({ title, description }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto text-center mb-6 md:mb-12 fade-in-section px-2">
      <h2 id="construyendo-juntos-heading" className={`${isMobile ? 'text-2xl px-2' : 'text-4xl'} font-bold text-[#333333] relative`}>
        {title}
      ...
      <div className="w-20 h-1 bg-[#F5A034] mx-auto my-4 md:my-6"></div>
      <p className={`${isMobile ? 'text-base px-3' : 'text-lg'} text-gray-700 mb-6 md:mb-8`}>
        {description}
      </p>
    </div>
  );
